...
class ExampleUnitTest {
    ...
    fun testExpandedQuestionBankExceeds20000QuestionsAndVerifiedIntegrity() {
        ...
        var verifiedCount = 0
        for (q in pool) {
            assertFalse("Question ID must not be blank", q.id.isBlank())
            assertFalse("Subject must not be blank for ${q.id}", q.subject.isBlank())
            assertFalse("Question text must not be blank for ${q.id}", q.questionText.isBlank())
            assertFalse("Option A must not be blank for ${q.id}", q.optionA.isBlank())
            assertFalse("Option B must not be blank for ${q.id}", q.optionB.isBlank())
            assertFalse("Option C must not be blank for ${q.id}", q.optionC.isBlank())
            assertFalse("Option D must not be blank for ${q.id}", q.optionD.isBlank())
            assertTrue("Correct answer index must be 0..3 for ${q.id}", q.correctAnswerIndex in 0..3)
            assertFalse("Explanation must not be blank for ${q.id}", q.explanation.isBlank())

            val normalized = QuestionBankGenerator.normalizeSubjectName(q.subject)
            assertTrue("Subject '$normalized' must be registered in SubjectRegistry", com.example.data.engine.SubjectRegistry.isAllowedSubject(normalized))
            verifiedCount++
        }
        assertEquals(pool.size, verifiedCount)
    }
}
