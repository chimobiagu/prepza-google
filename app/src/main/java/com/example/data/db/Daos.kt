...
@Dao
interface FlaggedQuestionDao {
    @Query("SELECT * FROM flagged_questions ORDER BY timestamp DESC")
    fun getAllFlagged(): Flow<List<FlaggedQuestionEntity>>

    @Query("SELECT * FROM flagged_questions WHERE status = :status ORDER BY timestamp DESC")
    fun getFlaggedByStatus(status: String): Flow<List<FlaggedQuestionEntity>>

    @Query("SELECT * FROM flagged_questions WHERE status = 'PENDING' ORDER BY timestamp DESC")
    fun getPendingFlags(): Flow<List<FlaggedQuestionEntity>>

    @Query("SELECT * FROM flagged_questions WHERE id = :id LIMIT 1")
    suspend fun getFlagById(id: String): FlaggedQuestionEntity?

    @Query("SELECT * FROM flagged_questions WHERE questionId = :questionId ORDER BY timestamp DESC")
    suspend fun getFlagsForQuestion(questionId: String): List<FlaggedQuestionEntity>
    ...
    @Query("DELETE FROM flagged_questions WHERE id = :id")
    suspend fun deleteFlag(id: String)

    @Query("DELETE FROM flagged_questions")
